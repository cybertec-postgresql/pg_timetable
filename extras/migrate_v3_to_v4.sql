-- This script should be executed on the latest v3 schema under the pg_timetable working role.
-- It produces `timetable_v3` backup schema with old data and new `timetable` v4 schema with migrated chains.
-- Last step is to run `pg_timetable --upgrade` and the schema will be upgraded to the latest one available.

BEGIN;

-- rename the schema to keep old data
ALTER SCHEMA timetable RENAME TO timetable_v3;
...
-- migrate data from v3 to v4 schema
INSERT INTO timetable.task (
	...
	t.chain_id AS task_id,
	t.parent_id,
	bt.name AS task_name,
	bt.kind :: TEXT :: timetable.command_kind,
	bt.script AS command,
	t.run_uid AS run_as,
	dc.connect_string AS database_connection,
	t.ignore_error,
	t.autonomous,
	0 AS timeout
FROM
	timetable_v3.task_chain t JOIN timetable_v3.base_task bt USING (task_id)
	LEFT JOIN timetable_v3.database_connection dc USING (database_connection);
...
INSERT INTO timetable.chain (
	...
	c.client_name
FROM
	timetable_v3.chain_execution_config c JOIN timetable_v3.task_chain t USING (chain_id);
...
INSERT INTO timetable.parameter (
	...
	order_id,
	value
FROM timetable_v3.chain_execution_parameters;
	
-- output migrated data from v4 schema for check
