...
CREATE OR REPLACE FUNCTION timetable.cron_months(from_ts timestamptz, allowed_months int[]) RETURNS SETOF timestamptz AS $$
	WITH 
...
RETURNS SETOF timestamptz AS $$
	...
	SELECT ts FROM gend JOIN ad ON date_part('day', gend.ts) = ad.ad WHERE extract(dow from ts)=ANY(allowed_week_days)
$$ LANGUAGE SQL STRICT;

CREATE OR REPLACE FUNCTION timetable.cron_times(allowed_hours int[], allowed_minutes int[]) RETURNS SETOF time AS $$
	WITH 
...
CREATE OR REPLACE FUNCTION timetable.cron_runs(from_ts timestamp with time zone, cron text) RETURNS SETOF timestamptz AS $$
	SELECT cd + ct 
	FROM 
