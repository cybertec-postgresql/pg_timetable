...
func TestTaskSendMail(t *testing.T) {
	...
	assert := assert.New(t)
	assert.Error(SendMail(EmailConn{}), `unexpected end of JSON input`,
		"Sending mail with empty param should fail")
	// assert.EqualError(SendMail(ctx, `{"ServerHost":""}`),
	// 	"The IP address or hostname of the mail server not specified", "Sending mail without host/IP should fail")
	// assert.EqualError(SendMail(ctx, `{"ServerHost":"smtp.example.com","ServerPort":0}`),
	// 	"The port of the mail server not specified", "Sending mail without port should fail")
	// assert.EqualError(SendMail(ctx, `{"ServerHost":"smtp.example.com","ServerPort":587,"Username":""}`),
	// 	"The username used for authenticating on the mail server not specified", "Sending mail without valid user id should fail")
	// assert.EqualError(SendMail(ctx, `{"ServerHost":"smtp.example.com","ServerPort":587,"Username":"user","Password":""}`),
	...
	// assert.EqualError(SendMail(ctx, `{"ServerHost":"smtp.example.com","ServerPort":587,"Username":"user","Password":"pwd","SenderAddr":""}`),
	// 	"Sender address not specified", "Sending mail without a valid sender address should fail")
	// assert.EqualError(SendMail(ctx, `{"ServerHost":"smtp.example.com","ServerPort":587,"Username":"user","Password":"pwd",
	...
	// assert.NoError(SendMail(ctx, `{"ServerHost":"smtp.example.com","ServerPort":587,"Username":"user","Password":"pwd",
